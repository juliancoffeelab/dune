Show that synthetic package paths remain queryable and also accept
origin context.
...
The consumer's Merlin config should point into the synthetic package tree.
...
The synthetic dependency file should be queryable directly.

  $ PKG_DIR=$(get_build_pkg_dir smoke_dep)
  $ SYNTH=$PWD/$PKG_DIR/source/smoke_dep.ml
  $ printf "(4:File%d:%s)" ${#SYNTH} "$SYNTH" | $dune ocaml-merlin \
...
The same synthetic path should also accept an explicit origin context.

  $ MAIN=$PWD/app/main.ml
  $ printf '(4:File%d:%s7:Context%d:%s)' ${#SYNTH} "$SYNTH" ${#MAIN} "$MAIN" \
  > | $dune ocaml-merlin \
  > | grep -o -E '_build/_private/default/.pkg/[^)]*/target/lib/smoke_dep|_build/_private/default/.pkg/[^)]*/source|ERROR' \
  > | sanitize_pkg_digest smoke_dep.0.0.1 \
  > | dune_cmd subst "$PWD" '$TESTCASE_ROOT'
